...
class BedrockService {
  ...
  async generateMindMapInsights(
    request: MindMapAnalysisRequest
  ): Promise<MindMapInsights> {
    try {
      console.log(
        "🧠 Generating AI mind map insights for:",
...
`;
  ...
  private parseTextResponse(
    ...
    let explanation =
      text.trim() ||
      `This legal text: "${selectedText}" requires professional interpretation for your specific situation.`;
  ...
  private addBasicBoldFormatting(text: string): string {
    ...
    const patterns = [
      {
        regex: /\b(EPF|SOCSO|SSM|Companies Commission)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex:
          /\b(Employment Act|Partnership Act|Contracts Act|Uniform Partnership Act)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex: /\b(shall|must|required|obligation|mandatory)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex: /\b(rights?|entitled|may|can)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex: /\b(confidential|non-compete|termination|partnership)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex:
          /\b(Malaysia|Malaysian|business purpose|capital contributions)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex: /\b(terms and conditions|principal place of business)\b/gi,
        replacement: "<strong>$1</strong>",
      },
      {
        regex:
          /\b(legal obligation|business partnership|social security|retirement savings)\b/gi,
        replacement: "<strong>$1</strong>",
      },
    ];
  ...
  private validateCategory(category: string): TextAnalysisResponse["category"] {
    ...
    return validCategories.includes(
      category as TextAnalysisResponse["category"]
    )
      ? (category as TextAnalysisResponse["category"])
      : "general";
  }
...
${conversations
  .map(
    (conv) =>
      `- ID: ${conv.id}, Title: "${conv.title}", Messages: ${conv.messages.length}`
  )
  .join("\n")}
  ...
  .map(
    (doc) =>
      `- ID: ${doc.id}, Filename: "${doc.originalFilename}"${
        doc.contentSummary ? `, Summary: "${doc.contentSummary}"` : ""
      }`
  )
  .join("\n")}
...
${actionItems
  .map(
    (action) =>
      `- ID: ${action.id}, Title: "${action.title}", Priority: ${action.priority}, Status: ${action.status}, Description: "${action.description}"`
  )
  .join("\n")}
...
`;
  ...
  private fallbackMindMapAnalysis(
    request: MindMapAnalysisRequest
  ): MindMapInsights {
    // Basic fallback analysis when AI is unavailable
    const urgentActions = request.actionItems.filter(
      (action) => action.priority === "urgent"
    );
    const pendingActions = request.actionItems.filter(
      ...
      riskFactors:
        urgentActions.length > 0
          ? [
              {
                description:
                  "Multiple urgent actions may indicate time-sensitive legal matters",
  ...
  private fallbackAnalysis(selectedText: string): TextAnalysisResponse {
    ...
    if (lowerText.includes("epf")) {
      explanation =
        "<strong>EPF</strong> is a <strong>mandatory retirement savings scheme</strong> in Malaysia where both employer and employee contribute.";
      category = "legal-term";
    } else if (lowerText.includes("socso")) {
      explanation =
        "<strong>SOCSO</strong> provides <strong>social security protection</strong> to employees through employment injury and pension schemes.";
      category = "legal-term";

...
function shouldSimulateError(): { shouldError: boolean; errorType?: 'network' | 'server' | 'validation' } {
...
export class MockClient {
  ...
  async request<T>(
    ...
    if (options.progressId && options.progressSteps) {
      progressTracker.simulateProgress(options.progressId, delayMs, options.progressSteps)
    }
  ...
  async processWithAI(
    ...
    const resultId = `${type}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    
    const stages = type === 'analysis' 
      ? ['Parsing input...', 'Analyzing content...', 'Identifying risks...', 'Generating recommendations...', 'Finalizing analysis...']
      : ['Understanding requirements...', 'Generating draft...', 'Reviewing content...', 'Adding suggestions...', 'Finalizing document...']
  ...
  async batchRequest<T>(
    ...
    const { concurrency = 3, delayBetween = 100, stopOnError = false } = options

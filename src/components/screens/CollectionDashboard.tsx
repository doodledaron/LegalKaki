...
import { ArrowLeft, Grid, List, CheckCircle, Clock, AlertTriangle, Eye, ExternalLink, ClipboardList, FileText, Calendar, MessageCircle, Brain, FileIcon, FileSpreadsheet, ImageIcon, Loader2, Trash2, Sparkles, X, Download } from 'lucide-react'
...
export function CollectionDashboard({ collectionId, onBack, onStartNewChat, onViewConversation }: CollectionDashboardProps) {
  ...
  const uploadedDocs = documents.filter(d => d.metadata?.sourceType !== 'draft');
  const draftDocs = documents.filter(d => d.metadata?.sourceType === 'draft');
  ...
  return (
    <div className="min-h-screen bg-background p-4 pb-nav">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="heading-3">Generated Drafts</h2>
            </div>
          </div>
          ...
          {draftDocs.length === 0 ? (
            <Card className="text-center py-8">
              <CardContent>
                ...
                <h3 className="heading-3 mb-2">No generated drafts</h3>
                <p className="body-regular text-text-secondary mb-4">
                  Drafts generated by the AI will appear here after you save them
                </p>
              </CardContent>
            </Card>
          ) : (
            <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-6' : 'space-y-4'}>
              {draftDocs.map((document, index) => (
                <motion.div
                  key={document.id}
                    ...
                    <CardContent className="relative p-6 z-10">
                      <div className="flex items-start space-x-4">
                        <div className="mt-1 transform group-hover:scale-110 transition-transform duration-300">
                          <FileText className="w-10 h-10 text-purple-primary group-hover:text-purple-light transition-colors duration-300" />
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <h4 className="heading-4 text-text-primary group-hover:text-purple-primary transition-colors duration-300">
                              {document.originalFilename}
                            </h4>
                          </div>

                          <p className="body-regular text-text-secondary mb-4 leading-relaxed">
                            {document.contentSummary || "No summary available for this draft."}
                          </p>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              <FileText className="w-6 h-6 text-purple-primary" />
              <h2 className="heading-3">Uploaded Documents</h2>
            </div>
              ...
              {uploadedDocs.length > 0 && (
                <>
                  ...
                  <Button
                    variant="primary"
                    size="small"
                    onClick={() => setSelectedDocument(uploadedDocs[0])}
                    leftIcon={<Eye className="w-4 h-4" />}
          ...
          {uploadedDocs.length === 0 ? (
            <Card className="text-center py-8">
              <CardContent>
                ...
                <h3 className="heading-3 mb-2">No uploaded documents</h3>
                <p className="body-regular text-text-secondary mb-4">
                  This collection does not contain any uploaded documents yet
                </p>
              </CardContent>
            </Card>
          ) : (
      ...
      {/* Document Viewer Modal - handles both PDFs and text files */}
      {selectedDocument && selectedDocument.fileType === 'application/pdf' && (
        <PDFViewer
...
// TextViewer component for displaying text-based documents (drafts)
interface TextViewerProps {
  document: Document
  onClose: () => void
}

function TextViewer({ document, onClose }: TextViewerProps) {
  return (
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="caption text-text-secondary">Draft Document</p>
            </div>
          </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 text-text-secondary body-small">
              <span>AI-generated draft document</span>
            </div>
            <Button variant="secondary" size="small" leftIcon={<Download className="w-4 h-4" />}>
              Download
            </Button>
          </div>
        </div>
